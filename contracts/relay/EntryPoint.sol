...
contract EntryPoint is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_registry != address(0), "EntryPoint: Invalid registry address");
        require(_initialOwner != address(0), "EntryPoint: Invalid owner address");
        require(_initialFeePercentage <= 1000, "EntryPoint: Fee percentage too high (max 10%)");
    ...
    function updateRegistry(address _newRegistry) external onlyOwner {
        require(_newRegistry != address(0), "EntryPoint: Invalid registry address");
    ...
    function updateServiceFee(uint256 _newFeePercentage) external onlyOwner {
        require(_newFeePercentage <= 1000, "EntryPoint: Fee percentage too high (max 10%)");
    ...
    function subscribeViaUrl(
        ...
        require(relayAddress != address(0), "EntryPoint: Relay not found for URL");
        require(registry.isRelayActive(relayAddress), "EntryPoint: Relay is not active");
    ...
    function subscribeDirect(
        ...
        require(_relayAddress != address(0), "EntryPoint: Invalid relay address");
        require(_months > 0, "EntryPoint: Months must be positive");
        ...
        if (isRegistered) {
            require(registry.isRelayActive(_relayAddress), "EntryPoint: Relay is not active");
        } else {
            // Se non è registrato, permetti comunque l'iscrizione ma verifica che sia un contratto
            require(Address.isContract(_relayAddress), "EntryPoint: Address is not a contract");
        }
    ...
    function batchSubscribe(
        ...
        require(_relayAddresses.length > 0, "EntryPoint: Empty relay addresses array");
        require(_relayAddresses.length == _pubKeys.length, "EntryPoint: Arrays must have same length");
        require(_months > 0, "EntryPoint: Months must be positive");
        ...
        for (uint256 i = 0; i < _relayAddresses.length; i++) {
            ...
            require(relayAddress != address(0), "EntryPoint: Invalid relay address");
            ...
            if (isRegistered) {
                ...
                require(Address.isContract(relayAddress), "EntryPoint: Address is not a contract");
            }
        ...
        require(msg.value >= totalAmount, "EntryPoint: Insufficient funds sent");
        ...
        for (uint256 i = 0; i < _relayAddresses.length; i++) {
            ...
            IRelay(relayAddress).subscribe{value: subscriptionCost}(_months, pubKey);
        }
    ...
    function _processSubscription(
        ...
        // Esegui l'iscrizione al relay
        IRelay(_relayAddress).subscribe{value: subscriptionCost}(_months, _pubKey);
    ...
    function hasRegisteredPubKey(address _user, address _relayAddress) external view returns (bool) {
        ...
        (uint256 expires, bytes memory pubKey) = IRelay(_relayAddress).getUserSubscriptionInfo(_user);
     ...
     */
    function getSubscriptionDetails(address _user, address _relayAddress) external view returns (
        uint256 expires,
        bytes memory pubKey
    ) {
        require(_relayAddress != address(0), "EntryPoint: Invalid relay address");
    ...
    function batchCheckPubKeys(address _user, address[] calldata _relayAddresses) external view returns (bool[] memory) {
        ...
        for (uint256 i = 0; i < _relayAddresses.length; i++) {
            address relayAddress = _relayAddresses[i];
            if (relayAddress != address(0)) {
                (, bytes memory pubKey) = IRelay(relayAddress).getUserSubscriptionInfo(_user);
